...
def entryDataManualy():
	...
	nameEntered0=ttk.Entry(labelsFrame,width=60,textvariable=name0)
	nameEntered0.grid(column=1,row=2)
	nameEntered0.focus()
	aLabel1=ttk.Label(labelsFrame,text="Number of Virtual Machines: ")
	aLabel1.grid(column=0,row=4)
	nameEntered1=ttk.Entry(labelsFrame,width=60,textvariable=name1)
...
def entry():
	...
	f.write(str(cloudlet)+"\n")
	f.write(str(vm)+"\n")
	for i in range(cloudlet):
