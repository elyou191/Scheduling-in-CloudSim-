...
def entryDataManualy():
	...
	labelsFrame=ttk.LabelFrame(win,text='Select data')
...
def run():
	...
	if(input_data):
		...
		execute_java(file_name,input_data,"C:\\Users\Youssef\eclipse-workspace1\cloudsim-3.0.3","cloudsim-3.0.3.jar",tab1,"SJF")
		#FCFS
		file_name = 'FCFS.java'
		compile_java(file_name,"C:\\Users\Youssef\eclipse-workspace1\cloudsim-3.0.3","cloudsim-3.0.3.jar")
		...
		if(msg_ans):
			...
			if(input_data):
				...
				tk.messagebox.showerror(title="Input", message="Data not selected")
...
btn_plot = tk.Button(fr_buttons, text="Show plot",command=showPlot)
...
btn_plot.grid(row=3,column=0,sticky="ew",ipadx=15,ipady=5,padx=20,pady=10)
btn_clr.grid(row=4,column=0,sticky="ew", ipadx=15,ipady=5,padx=20, pady=10)
btn_save.grid(row=5, column=0, sticky="ew", ipadx=15,ipady=5,padx=20, pady=10)
btn_exit.grid(row=6,column=0,sticky="ew",ipadx=15,ipady=5,padx=20,pady=10)
